...
const fetchFiles = async () => {
  ...
  try {
    ...
    // Fix: Access the files array from the response
  ...
  const handleStarFile = async (fileId: string) => {
    try {
      const headers = await getAuthHeaders();
      await axios.patch(`/api/files/${fileId}/star`, {}, { headers });
  ...
  const handleTrashFile = async (fileId: string) => {
    try {
      const headers = await getAuthHeaders();
      const response = await axios.patch(`/api/files/${fileId}/trash`, {}, { headers });
  ...
  const handleDeleteFile = async (fileId: string) => {
    try {
      ...
      // Send delete request
      const headers = await getAuthHeaders();
      const response = await axios.delete(`/api/files/${fileId}/delete`, { headers });
  ...
  const handleEmptyTrash = async () => {
    try {
      const headers = await getAuthHeaders();
      await axios.delete(`/api/files/empty-trash`, { headers });
